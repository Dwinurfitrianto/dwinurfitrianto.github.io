...
export default {
  ...
  methods: {
    getChapter(){
      fetch('https://api.quran.com/api/v4/chapters/108?language=id',{
        method: 'GET'
      })
        .then(response => {
          if(response.ok){
    ...
    getQuran(){
      fetch('https://api.quran.com/api/v4/quran/verses/uthmani?chapter_number=108&juz_number=30', {
        method: 'GET'
      })
        .then(res =>{
          if(res.ok){
    ...
    getTranslate() {
      fetch('https://api.quran.com/api/v4/quran/translations/33?chapter_number=108&juz_number=30', {
        method: 'GET'
      })
        .then(response => {
          if (response.ok) {
    ...
    getAudio() {
      fetch('https://api.quran.com/api/v4/chapter_recitations/5/108', {
        method: 'GET'
      })
        .then(response => {
          if (response.ok) {
    ...
    getInfo() {
      fetch('https://api.quran.com/api/v4/chapters/108/info?language=id', {
        method: 'GET'
      })
        .then(response => {
          if (response.ok) {
